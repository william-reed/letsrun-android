 ...
 */
class ForumService(private val objectBox: ObjectBox) {
    // box for thread replies. aka cached replies
    private val replyBox = objectBox.boxStore.boxFor(ThreadReply::class.java)
    ...
    fun fetchThreadReplies(threadId: Int, page: Int = 0): Single<List<ThreadReply>> =
        ...
        Single.fromCallable {
            ...
            .map { it.attr("name") }
            // get the reply data
            .flatMapSingle { fetchReply(it.toLong()) }
            .toList()

    /**
     * Fetch a specific reply. Return a cached version if available
     *
     * @param replyId the reply id generated by letsrun
     */
    private fun fetchReply(replyId: Long): Single<ThreadReply> {
        val reply = replyBox.get(replyId)
        return if (reply != null) {
            Single.just(reply)
        } else {
            Single.fromCallable { Jsoup.connect("https://www.letsrun.com/forum/posts/$replyId/reply").get() }
                .subscribeOn(Schedulers.io())
                .observeOn(Schedulers.computation())
